...
public class ViewUtilities {
     ...
     * boolean is set to true. If the current scheme is dark, the image is searched in /res.pictures/icons/dark,
     * else in /res.pictures/icons/light and if the icon is ot found then it is searched in /res.pictures/icons.
    ...
    public static ImageIcon getSchemeIcon(String iconName, boolean manageSize) {
        Image image;
        if (Scheme.isDark()) image = getImage("/res/pictures/icons/dark/" + iconName);
        else image = getImage("/res/pictures/icons/light/" + iconName);
        if (image == null) image = getImage("/res/pictures/icons/" + iconName);
        assert image != null;
        if (manageSize){
